...
<html>
    <head>
        <meta charset="utf-8">
        <title>Redirect to Tidio Automation...</title>
    </head>
    <body>
        <?php /* ACCESS REDIRECT */ ?>
        ...
        <?php if ($view['mode'] == 'access_request') { ?>
            ...
            <script>
                ...
                jQuery.getJSON('<?php echo $view['access_url'] ?>&remote=1', function (data) {
                    ...
                    data = data['value'];
                    location.href = path.admin_url + 'admin-ajax.php?action=tidio_automation_redirect&access_status=success&private_key=' + data.private_key + '&public_key=' + data.public_key;
                });
            </script>
        <?php } ?>
    </body>
