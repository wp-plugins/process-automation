var tidioAutomationOptions = {
	...
	create: function(data){
		...
		this.registerProject(function(){
			...
			tidioAutomationOptions.automationLoading();
	...
	initEvents: function(){
		...
		$("#automation-settings-link").on('click', function(){
	...
	apiUpdateSettings: function(_func){
		...
		var plugin_data = tidioAutomationOptions.apiDataSerialize('#dialog-settings-form');
		...
		$.ajax({
			url: this.ajax_url + 'admin-ajax.php?action=tidio_automation_settings_update',
			type: 'POST',
			data: {
	...
	automationLoading: function(){
		
		var iframe_height = $('body').height() - ($("#wpadminbar").height() + $(".wrap h2").height() + 10);
		
		
		
		$("#automation-content").html(
			'<iframe src="' + this.plugin_url + 'en/panel/plugins/automation/external?externalAccessKey=' + this.private_key + '" id="automation-iframe" style="height: ' + iframe_height + 'px;"></iframe>'
		);		
		
		$("#automation-iframe").load(function(){
			
			$("#automation-loading").fadeOut('fast', function(){
				
				$("#automation-content").fadeIn('fast');
				
				tidioAutomationOptions.automationIsReady();
